...
- **PWA-only files (not mirrored to extension):** `lock.html`, `nosw.html` (one-tap unregister of the service worker + clear `caches` API storage, then reload `index.html` — use if the PWA shows a blank white screen after an update), `service-worker.js`, `manifest.webmanifest`, and the `?debug=1` bypass in `index.html` exist because the PWA has a separate lock screen and offline cache to manage. The extension has none of those concerns (Chrome handles install/auth and the SW is replaced by `background.js`), so they intentionally have no counterpart in `finance-tracker-chrome/`.
