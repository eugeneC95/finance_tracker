...
function openCatDetail(cat, isIncome) {
  ...
  sorted.forEach(entry => {
    const dlbl = new Date(entry.date+'T00:00:00').toLocaleDateString('en-MY',{month:'short',day:'numeric',year:'numeric'});
...
function renderTxList(id, items, catMap, isIncome) {
  ...
  items.forEach(entry => {
    ...
    const dlbl     = new Date(entry.date+'T00:00:00').toLocaleDateString('en-MY',{month:'short',day:'numeric'});
